"""
cleanup_data.py
Hand-curated data used by 4_cleanup_and_prepare.py.

This file holds three datasets that grow over time as you discover edge
cases in the LLM extraction:

  KNOWN_DUPLICATES    — variant spellings → canonical name
  TO_BE_EXCLUDED      — names of non-criminal entities that slip through
  NODE_TYPE_OVERRIDES — per-organization type corrections

Edit this file directly when you find new cases. The cleanup script
re-reads it on every run; no other code changes are needed.
"""
...
# ═══════════════════════════════════════════════════════════════════
# NODE_TYPE_OVERRIDES
# Lowercase organization name → canonical type.
# Used when the LLM consistently misclassifies a specific organization.
# Applied AFTER dedup, so use the canonical name as the key.
# ═══════════════════════════════════════════════════════════════════
